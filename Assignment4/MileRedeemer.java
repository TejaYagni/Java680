 ...
 *		mileage into airline tickets through GUI    *
 ***********************************************************/
...
class MileRedeemer
{
	...
	for(Destination d: destinationList) 
	    {
    		...
    		else if(month >= d.getStartMonth() && month <= d.getEndMonth()) //If month is in super saver miles then this block is executed
		    {
			    ...
			    {
				/* If miles is greater than super saver then the miles is decresed by super saver ad the current instance of Destiantion is added into array list abc */
    				abc.add(d); 
    ...
    public Destination findDestination(String cityName)
    {
    	...
    	for( Destination d: destinationList)
    	{
    		if(d.getDestinationCity() == cityName)
    			temp = d;
    	}
